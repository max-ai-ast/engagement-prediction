...
- **Do not duplicate defaults** in stage scripts, dataloaders, or model classes. The only place to define defaults for pipeline parameters is `cli.py`.
- When adding a new CLI-controlled hyperparameter: add it to `DEFAULTS` in `cli.py`, add the corresponding `--flag` to the parser, and pass the value from `args` into the model/encoder constructors without defining a default in those constructors.
