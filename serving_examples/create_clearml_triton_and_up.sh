...
Options:
  ...
  --serving-id <id>       Existing serving service id (optional; otherwise read from serving_examples/docker.env)
...
if [[ -z "$preprocess_path" ]]; then
  if [[ "$model_type" == "mlp" ]]; then
    preprocess_path="serving_examples/preprocess_mlp.py"
  elif [[ "$model_type" == "post" ]]; then
    preprocess_path="serving_examples/preprocess_post.py"
  elif [[ "$model_type" == "user" ]]; then
    preprocess_path="serving_examples/preprocess_user.py"
  fi
fi
